  ...
  if ($do_skip) {
    ...
    if (m@^([_a-zA-Z][.\w]*)\s*:$@) {  # Label.
      ...
      if (($inst eq "jmp" or $inst eq "jmpl" or $inst eq "call" or $inst eq "calll") and @args == 1) {
        ...
        if (m@^movb %al, ([_a-zA-Z][.\w]*)$@) {  # !!! optimization !! comment;
          ($_, $prews) = ("$prews.byte 0xa2  # movb %al, $1\n$prews.long $1", "");
        } elsif (m@^cmp \$(.*?), %bx$@) {  # Bugfix for GNU as(1) 2.9.1. It's already fixed in GNU as(1) 2.9.5.
          ($_, $prews) = ("$prews.byte 0x66, 0x81, 0xfb  # cmp \$$1, %bx\n$prews.word $1", "");
        } elsif (m@^cmp \$(.*?), %ax$@) {  # Bugfix for GNU as(1) 2.9.1. It's already fixed in GNU as(1) 2.9.5.
